...
interface LearningItem {
  ...
  choice3: string
  choice4: string
  correct_index: number // 0: 보기1, 1: 보기2, 2: 보기3, 3: 보기4
  image_url?: string
...
export function LearningManagement({
  ...
  const handleSubmit = async () => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      // 정답 인덱스 검증
      if (item.correct_index < 0 || item.correct_index > 3) {
        toast({
          title: "오류",
          description: `문항 ${i + 1}: 정답을 선택해주세요.`,
          variant: "destructive",
        })
        return
      }
    ...
    try {
      ...
      const itemsToSubmit = await Promise.all(
        items.map(async (item) => {
          ...
          // correct_index 직접 사용
          if (item.correct_index < 0 || item.correct_index > 3) {
            throw new Error(`문항 ${items.indexOf(item) + 1}: 정답 인덱스가 유효하지 않습니다.`)
          }
    ...
    const template = [
      {
        ...
        choice3: "오렌지",
        choice4: "포도",
        correct_index: 0, // 0: 보기1, 1: 보기2, 2: 보기3, 3: 보기4
      },
      {
        type: "TYPE_B",
        word_text: "book",
    ...
    try {
      const itemsToSubmit = await Promise.all(
        editItems.map(async (item) => {
          ...
          // correct_index 직접 사용
          if (item.correct_index < 0 || item.correct_index > 3) {
            throw new Error(`문항 ${editItems.indexOf(item) + 1}: 정답 인덱스가 유효하지 않습니다.`)
          }
