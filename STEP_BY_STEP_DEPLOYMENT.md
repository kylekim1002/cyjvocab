...
4. **Build Command**: **중요!** 저장소의 `vercel.json`이 적용되면 다음과 같습니다:
   ...
   ```
   ⚠️ `prisma db push`는 Supabase **pooler(6543)** 환경에서 빌드가 멈출 수 있어 빌드에 포함하지 않습니다. DB 스키마는 SQL Editor 또는 로컬에서 Direct URL로 `db push` 하세요.
5. **Output Directory**: `.next` (기본값)
6. **Install Command**: `npm install` (기본값)

**⚠️ 중요: Build Command 확인**
- `prisma generate && next build`가 적용되는지 확인하세요 (`vercel.json`)
