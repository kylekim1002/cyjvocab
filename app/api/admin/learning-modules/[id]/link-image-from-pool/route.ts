...
export async function POST(
  ...
  try {
    ...
    if (module.type !== "TYPE_B") {
      return NextResponse.json(
        { error: "이미지 풀 자동 연결은 TYPE_B(그림+단어+뜻) 학습에서만 사용할 수 있습니다." },
        { status: 400 }
      )
    }
    ...
    const pool = await prisma.wordImage.findMany({
    ...
    for (const item of module.items) {
      ...
      linked++
      if (p.image_url === url) continue
      const next: Payload = { ...p, image_url: url }
      updates.push(
    ...
    return NextResponse.json({
      ...
      message:
        uniqueMissing.length === 0
          ? `모든 문항에 이미지가 연결되었습니다. (${linked}개)`
          : `연결됨 ${linked}개. 풀에 없는 키: ${uniqueMissing.length}개`,
    })
  } catch (e: any) {
    console.error("link-image-from-pool:", e)
    return NextResponse.json(
      { error: wordImageTableMissingMessage(e.message || "자동 연결에 실패했습니다.") },
      { status: 500 }
    )
  }
}
